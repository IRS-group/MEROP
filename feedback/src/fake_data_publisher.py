...
class DataGenerator:
    ...
    def __init__(self):
        ...
        self.full_rotation_duration = rospy.get_param('~full_rotation_duration', 5.0)
        self.vibration_duration_param = rospy.get_param('~vib_duration', 5.0)
        self.traction_duration_last_change = rospy.get_time()

        self.current_traction_state = -1
        self.last_vibration_state = 1
    ...
    def start_data_generator(self):
        ...
        while not rospy.is_shutdown():
            ...
            traction_state = self.publish_traction_better(traction_state)
    ...
    def publish_traction_better(self, traction_state):
        """
        Traction States meaning:
            1 = stuck
            2 = sliding
        ...
        if self.current_traction_state == -1 and (rospy.get_time() - self.traction_duration_last_change) > 1.5:
            if self.last_vibration_state == 1:
                traction_state = 2
            else:
                traction_state = 1
            self.last_vibration_state = traction_state
            # reset the timer for the traction duration
            self.traction_duration_last_change = rospy.get_time()

        if self.last_vibration_state == 2 or self.last_vibration_state == 1:
            if (rospy.get_time() - self.traction_duration_last_change) > self.vibration_duration_param:
                traction_state = -1
                self.traction_duration_last_change = rospy.get_time()
        ...
        #     previous = traction_state
        #     # ensure that the random pic is always different from the last one
        #     while previous == traction_state:process has finished cleanly
        ...
        self.current_traction_state = traction_state
        # publish traction data/state
        self.pub_traction.publish(Int16(traction_state))
